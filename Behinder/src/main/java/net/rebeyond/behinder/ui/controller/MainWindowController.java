...
public class MainWindowController {
    ...
    private void initControls() {
        ...
        if (labelCount > 1) {
            ...
            taskCenterLabel.setOnMouseClicked((event) -> this.showTaskCenter());
        }
...
//                                    MainWindowController.this.userCodeViewController.init(MainWindowController.this.currentShellService, MainWindowController.this.workList, MainWindowController.this.statusLabel);
                                    MainWindowController.this.userCodeViewController.init(MainWindowController.this.currentShellService, MainWindowController.this.workList, MainWindowController.this.statusLabel, MainWindowController.this.shellManager);
                            ...
                            public void run() {
                                while(true) {
                                    try {
                                        ...
                                        Platform.runLater(() -> Utils.showErrorMessage("提示", "由于您长时间未操作，当前连接会话已超时，请重新打开该网站。"));
                                        return;
                                    }
                                }
                            }
                        ...
                        Platform.runLater(() -> {
                            ...
                            this.statusLabel.setText(decryptException.getMessage());
                            Hyperlink detailLink = new Hyperlink("点击查看响应详情");
                            detailLink.setOnMouseClicked((event) -> Utils.showErrorMessage("响应内容", decryptException.getResponseBody()));
                            Separator separator = new Separator();
                            separator.setOrientation(Orientation.VERTICAL);
                            separator.setPrefHeight((double)20.0F);
                        ...
                        Platform.runLater(() -> {
                            this.connStatusLabel.setText("连接失败");
                            this.connStatusLabel.setTextFill(Color.RED);
                            this.statusLabel.setText("[ERROR]连接失败：" + e.getClass().getName() + ":" + e.getMessage());
                        });
    ...
    public void init(JSONObject shellEntity, ShellManager shellManager, Map<String, Object> currentProxy, boolean offline) throws Exception {
    ...
    public void init(JSONObject shellEntity, BShell bShell, ShellManager shellManager, Map<String, Object> currentProxy, List<Map<String, Object>> childList) throws Exception {
        this.shellEntity = shellEntity;
    ...
    private void initTabs() {
        if (this.effectShellEntity.getString("type").equals("asp")) {
            for(Tab tab : this.mainTabPane.getTabs()) {
                if (tab.getId().equals("realCmdTab") || tab.getId().equals("tunnelTab") || tab.getId().equals("parallelViewTab") || tab.getId().equals("reverseTab")) {
                    tab.setDisable(true);
                }
            }
        } else if (this.effectShellEntity.getString("type").equals("native")) {
            for(Tab tab : this.mainTabPane.getTabs()) {
                if (tab.getId().equals("tunnelTab") || tab.getId().equals("reverseTab") || tab.getId().equals("parallelViewTab") || tab.getId().equals("pluginViewTab") || tab.getId().equals("databaseTab") || tab.getId().equals("sourceCodeTab")) {
                    tab.setDisable(true);
                }
            }
        }
        ...
        for(Task task : this.taskList) {
            if (!(task.getProgress() >= (double)1.0F)) {
                ...
                taskStatus.setOnMouseClicked((event) -> {
                    Optional<ButtonType> buttonType = Utils.showConfirmMessage("提示", "是否确认终止该任务");
